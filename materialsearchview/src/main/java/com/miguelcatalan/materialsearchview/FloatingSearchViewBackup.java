...
public class FloatingSearchViewBackup extends FrameLayout implements Filter.FilterListener {
...
//                showSuggestions();
    ...
    public FloatingSearchViewBackup(Context context) {
        this(context, null);
    }

    public FloatingSearchViewBackup(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public FloatingSearchViewBackup(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs);
        mContext = context;
        initiateView();
    ...
    private void initiateView() {
        ...
        mSearchTopBar.setLayoutParams(new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, (int) DimensionUtil.dpToPx(mContext, 48)));
    ...
    private void initSearchViewListener() {
        ...
        mSearchSrcTextView.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                startFilter(s);
                FloatingSearchViewBackup.this.onTextChanged(s);
            }
    ...
    private void switchToSearchOrBackMode(int searchOrBack) {
        switch (searchOrBack) {
    ...
    private void setBackIcon(int type) {
    ...
    private void startFilter(CharSequence s) {
        if (mAdapter != null && mAdapter instanceof Filterable) {
            ((Filterable) mAdapter).getFilter().filter(s, FloatingSearchViewBackup.this);
        }
    }
    ...
    private void onTextChanged(CharSequence newText) {
        ...
        if (mOnSearchViewListener != null && !TextUtils.equals(newText, mOldQueryText)) {
            mOnSearchViewListener.onQueryTextChange(newText.toString());
        }
    ...
    private void onSubmitQuery() {
        ...
        if (query != null && TextUtils.getTrimmedLength(query) > 0) {
            if (mOnSearchViewListener != null) {
                mOnSearchViewListener.onQueryTextSubmit(query.toString());
            }
            switchToSearchOrBackMode(SEARCH);
        }
    }
...
/**
    ...
    public void showSuggestions() {
        if (mAdapter != null && mAdapter.getCount() > 0 && mSuggestionsListView.getVisibility() == GONE) {
            ...
            mSearchTopBar.setBackgroundResource(R.drawable.bg_rounded_top_layout);
        }
    }
...
/**
    ...
    private void setQuery(CharSequence query, boolean submit) {
